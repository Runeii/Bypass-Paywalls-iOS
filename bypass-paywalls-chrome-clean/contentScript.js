        ...
        if (json_script) {
          try {
            ...
            if (url_next && !window.location.pathname.endsWith(url_next))
              refreshCurrentTab();
            let json_text = findKeyJson(json, ['body', 'content', 'contentHtml', 'description'], 500);
            if (typeof json_text === 'string')
              json_text = parseHtmlEntities(json_text);
            else if (Array.isArray(json_text))
              json_text = '<p style="margin: 10px;">' + json_text.map(x => (typeof x === 'string') ? x : (x.children ? x.children.map(y => y.text).join('') : x.text)).join('<br><br>') + '</p>';
            if (json_text) {
              let parser = new DOMParser();
...
if (matchDomain(usa_adv_local_domains)) {
  ...
  if (window.location.pathname.match(/^\/(article|news)\//)) {
    ...
    if (article) {
      ...
      if (page_content && dompurify_loaded) {
