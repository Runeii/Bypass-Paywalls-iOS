if (!isDomainMatch(['seekingalpha.com', 'sfchronicle.com', 'cen.acs.org', 'elmundo.es'])) {
	window.localStorage.clear();
}

if (isDomainMatch('elmercurio.com')) {
...
else if (isDomainMatch('estadao.com.br')) {
	setTimeout(function () {
...
else if (isDomainMatch('repubblica.it')) {
	document.addEventListener('DOMContentLoaded', () => {
...
else if (isDomainMatch('americanbanker.com')) {
...
else if (isDomainMatch('telegraaf.nl')) {
...
else if (isDomainMatch(['ad.nl', 'ed.nl', 'bndestem.nl', 'bd.nl', 'tubantia.nl', 'destentor.nl', 'pzc.nl', 'gelderlander.nl'])) {
	const paywall = document.querySelector('.article__component.article__component--paywall-module-notification');
	removeDOMElement(paywall);
}
else if (isDomainMatch('washingtonpost.com')) {
...
else if (isDomainMatch('wsj.com') && !isDomainMatch('cn.wsj.com')) {
...
else if (isDomainMatch('sloanreview.mit.edu')) {
	const readMore = document.querySelector('.btn-read-more');
	if (readMore) {
		readMore.click();
	}
}
else if (isDomainMatch('mexiconewsdaily.com')) {
	window.setTimeout(function () {
...
else if (isDomainMatch('the-american-interest.com')) {
	const counter = document.getElementById('article-counter');
	removeDOMElement(counter);
}
else if (isDomainMatch('nzherald.co.nz')) {
...
else if (isDomainMatch(['parool.nl', 'trouw.nl', 'volkskrant.nl', 'demorgen.be', 'humo.be'])) {
	document.addEventListener('DOMContentLoaded', () => {
...
else if (isDomainMatch('firstthings.com')) {
	const paywall = document.getElementsByClassName('paywall')[0];
	removeDOMElement(paywall);
}
else if (isDomainMatch('bloomberg.com')) {
	blockElement('#graphics-paywall-overlay', true);
    blockElement('#fortress-paywall-container-root', true);
}
else if (isDomainMatch('bloombergquint.com')) {
	const articlesLeftModal = document.getElementsByClassName('paywall-meter-module__story-paywall-container__1UgCE')[0];
	const paywall = document.getElementById('paywallDmp');
	removeDOMElement(articlesLeftModal, paywall);
}
else if (isDomainMatch('medium.com') || document.querySelector('script[src^="https://cdn-client.medium.com/"]')) {
...
else if (isDomainMatch('theglobeandmail.com')) {
...
else if (isDomainMatch(['examiner.com.au', 'thecourier.com.au', 'theadvocate.com.au'])) {
...
else if (isDomainMatch('canberratimes.com.au')) {
...
else if (isDomainMatch('thehindu.com')) {
	document.addEventListener('DOMContentLoaded', () => {
...
else if (isDomainMatch('nytimes.com')) {
...
else if (isDomainMatch('technologyreview.com')) {
	window.setTimeout(function () {
...
else if (isDomainMatch('leparisien.fr')) {
...
else if (isDomainMatch('caixinglobal.com')) {
...
else if (isDomainMatch('bizjournals.com')) {
...
} else if (isDomainMatch('techinasia.com')) {
...
} else if (isDomainMatch('thewrap.com')) {
...
} else if (isDomainMatch('hbr.org')) {
	const banner = document.querySelector('.persistent-banner');
	removeDOMElement(banner);
} else if (isDomainMatch('spectator.co.uk')) {
	console.log('EXECUTED SCRIPT', window.location.href)
...
} else if (isDomainMatch('barrons.com')) {
	const url = window.location.href;
...
} else if (isDomainMatch('nzz.ch')) {
	const paywall = document.querySelector('.dynamic-regwall');
	removeDOMElement(paywall);
} else if (isDomainMatch('irishtimes.com')) {
	document.addEventListener('DOMContentLoaded', () => {
...
} else if (isDomainMatch('thesaturdaypaper.com.au')) {
	const paywall = document.querySelector('div.paywall-hard-always-show');
	removeDOMElement(paywall);
} else if (isDomainMatch('lesechos.fr') && window.location.href.match(/-\d{6,}/)) {
	window.setTimeout(function () {
...
} else if (isDomainMatch('startribune.com')) {
...
} else if (isDomainMatch('seattletimes.com')) {
	window.setTimeout(function () {
...
} else if (isDomainMatch('theatlantic.com')) {
...
} else if (isDomainMatch('newyorker.com')) {
	blockElement('.paywall-bar', true);
	blockElement('.paywall-modal');
} else if (isDomainMatch('vanityfair.com')) {
	const paywall = document.querySelector('.paywall-bar');
	removeDOMElement(paywall);
} else if (isDomainMatch(['postimees.ee', 'elu24.ee'])) {
	setTimeout(function () {
...
} else if (isDomainMatch('themarker.com')) {
	setTimeout(function () {
...
} else if (isDomainMatch('haaretz.co.il')) {
	setTimeout(function () {
		const notifications = document.querySelector('#pwSubscribePopup');
		const paywall = document.querySelector('[data-test="bottomStrip"]');
...
} else if (isDomainMatch('sueddeutsche.de')) {
	const url = window.location.href;
	document.addEventListener('DOMContentLoaded', () => {
...
} else if (isDomainMatch('adweek.com')) {
...
} else if (isDomainMatch('gelocal.it')) {
...
} else if (isDomainMatch('elmundo.es')) {
...
} else if (isDomainMatch('time.com')) {
	const body = document.querySelector('body');
	if (body) {
		body.setAttribute('style', 'position:relative !important;');
	}
} else if (isDomainMatch('expansion.com')) {
	const premium = document.querySelector('.ue-c-article__premium');
	const url = window.location.href;
...
} else if (isDomainMatch('chicagobusiness.com')) {
	const body = document.querySelector('body[class]');
	if (body) {
		body.removeAttribute('class');
	}
} else if (isDomainMatch('latimes.com')) {
...
} else if (isDomainMatch('foreignpolicy.com')) {
...
} else if (isDomainMatch('bostonglobe.com')) {
...
} else if (isDomainMatch('nationalgeographic.com')) {
